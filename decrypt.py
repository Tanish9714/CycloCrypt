...
def decrypt(ciphertext,k1,k2) -> str:
    ...
    print(f"k5: {k5}")
    plain_bin = strToBinary(ciphertext)
    print(f"plain_bin: {plain_bin}")

    k6 = sbox(k5, k4)
    print(f"k6: {k6}")
    plain = xor(k6, plain_bin, len(plain_bin))
    print(f"plain: {plain}")
    ...
    for i in range(0, len(lst_plain), 8):
        x = lst_plain[i:i + 8]
        x = ''.join(x)
        pla_lst_bin.append(x)

    pla_lst_dec = []
    for i in range(0, len(pla_lst_bin)):
        pla_lst_dec.append(bin2dec(int(pla_lst_bin[i])))

    print(pla_lst_dec)

    pla_lst_text = []
    for i in range(0, len(pla_lst_dec)):
        pla_lst_text.append(decToStr(pla_lst_dec[i]))

    plaintext = ''.join(pla_lst_text)
